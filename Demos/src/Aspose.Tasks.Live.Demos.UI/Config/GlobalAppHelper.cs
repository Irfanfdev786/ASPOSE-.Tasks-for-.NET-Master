...
namespace Aspose.Tasks.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeTasksContext.atcc = new AsposeTasksContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeTasksContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeTasksContext.Current.Cache.Add(
				   SessionID,
					resources,
