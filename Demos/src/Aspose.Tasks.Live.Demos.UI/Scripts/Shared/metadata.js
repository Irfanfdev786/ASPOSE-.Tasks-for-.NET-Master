(function ($) {
    $.metadata = function (data, fileId, fileName) {
        ...
        var display = function() {
            ...
            for (var i in items) {
				...
				if (items[i].Value === null) {
					tr.append($('<td data-item-value><span>' + '' + '</span></td>'));
				}
				else {
					tr.append($('<td data-item-value><span>' + items[i].Value + '</span></td>'));
				}
				if (items[i].Type === undefined) {
					tr.append($('<td data-item-type="' + items[i].Type + '"><span>' + TYPES[1] + '</span></td>'));
				}
				else {
        ...
        var onSave = function() {
            showLoader();
			const url = o.UIBasePath + 'api/AsposeTasksMetadata/download';
        ...
        var onClearAll = function() {
            if (window.confirm('Are you sure you want to clear all metadata?')) {
                showLoader();
				const url = o.UIBasePath + 'api/AsposeTasksMetadata/clear';
