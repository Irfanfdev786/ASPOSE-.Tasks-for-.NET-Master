...
function generateViewerLink(data) {
	var response = response = data.split('|');
...
function requestMetadata(data) {
	...
	if (response.length > 0) {
		let url = o.UIBasePath + 'api/AsposeTasksMetadata/properties?folderName=' + response[2] + "&fileName=" + response[1];
		$.ajax({
			type: 'POST',
			url: url,
...
function requestViewer(data) {
	...
	openIframe(url, '/tasks/viewer', '/tasks/view');
}
